...
ARKサーバーのMAPとCurseForge MODをWeb画面で編集する。
保存後は変更内容をARK通知チャンネルに投稿する。
サーバー内にプレイヤーがいない場合のみ、自動で再起動する。
...
Nitrado上のARK保存データと設定ファイルをバックアップする。
バックアップ結果はARK通知チャンネルに投稿する。
...
保存済みバックアップの一覧をWeb画面で開き、選択したバックアップを復元する。
復元は確認画面を挟み、バックアップ対象ファイルをバックアップ内容に置き換える。
サーバー内にプレイヤーがいる場合や人数を確認できない場合は実行しない。
復元後はサーバーを再起動する。
...
バックアップも同じチャンネルへ通知する。
自動バックアップは週1回実行し、サービス終了状態を検知した場合は最終バックアップを1回だけ試行する。
サービスが再開した場合は自動バックアップも再開する。
保存先は `ARK_BACKUP_DIR` で変更でき、未指定時は `data/ark-backups` に保存する。
...
開始時に現在取得できる最新画像を投稿し、その後は設定した頻度で新しい相場画像を投稿する。
...
相場画像には、`/poe2-market-edit` で選択したアイテムの高貴なオーブ換算と神のオーブ換算を表示する。
表示アイテムが未設定の場合は開始できないため、先に設定画面で選択する。

#### `/poe2-market-edit`

PoE2相場画像に表示するアイテムをWeb画面で設定する。
表示アイテムと投稿頻度（1時間～24時間）はDiscordサーバー内で共通となり、定期投稿中に変更した場合は次回の画像から反映する。
設定画面には、直近に保存したユーザーと設定内容の履歴を表示する。
ゲーム内の交換カテゴリ順で、カレンシー、エッセンス、デリリウム、ブリーチ、リチュアル、エクスペディション、アビス、インカージョン、フラグメント、ルーン、ソウルコア、アイドル、ジェムの原石、ジェム（リネージュサポート）を選択できる。
Web画面と相場画像のアイテム名には、取得できる場合は公式トレードサイトの日本語表記を使用する。
選択をすべて外して保存すると未設定状態になり、定期投稿中でも新しい画像の投稿を保留する。
...
募集Web画面またはPoE2相場設定画面を使用する場合は `ACT_WEB_BASE_URL` に外部から開けるURL、`ACT_WEB_SIGNING_SECRET` にランダムな秘密値を設定する。
