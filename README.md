...
開始時に現在取得できる最新画像を投稿し、その後は1時間単位の確定相場が更新された時に投稿する。
...
公式Currency Exchange APIを使用するため、`service:cxapi` が利用できる認証情報と対象リーグを `.env` に設定する。
`POE2_ACCESS_TOKEN` を設定するか、`POE2_CLIENT_ID` と `POE2_CLIENT_SECRET` を設定してBot側でトークンを取得する。
