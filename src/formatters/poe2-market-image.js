...
<svg width="${IMAGE_WIDTH}" height="${IMAGE_HEIGHT}" viewBox="0 0 ${IMAGE_WIDTH} ${IMAGE_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="642" y="169" fill="#8997aa" font-size="16" font-weight="700" font-family="${fontFamily()}">相場</text>
...
function buildProductRow(product, iconDataUrl, index, latestChangeId) {
    ...
    <text x="642" y="${y + 33}" fill="${product.lowestPrice === null ? '#728096' : '#f1c76e'}" font-size="20" font-weight="600" font-family="${fontFamily()}">${price}</text>
    ${staleLabel}
...
function formatPrice(product) {
    ...
    if (Math.abs(product.highestPrice - product.lowestPrice) < 0.0001) {
        return `${formatNumber(product.lowestPrice)} 高貴`;
    }

    return `${formatNumber(product.lowestPrice)} - ${formatNumber(product.highestPrice)} 高貴`;
}
